...
- Provide Git over SSH and HTTPS, repository browsing, diffs, simple issues, pull requests, reviews, and protected merges. Use the standard Git implementation for Git operations.
...
4. Expose repository discovery, issue and diff reads, pull-request writes, and review feedback through the MCP integration for Codex and Claude. Verify the protocol with the official SDK and verify configuration flags against the installed clients.
...
Go, Gleam, and Rust implementations are present, with one shared interface, independent backend logic, Docker packaging, and repeatable validation. Current results and measurement conditions are recorded in COMPARISON.md. Reliability and comparative performance claims remain limited to that evidence.
