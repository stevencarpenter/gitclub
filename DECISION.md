...
Completed 2026-09-14. The application code needed no restructuring: every row and parameter already passed through the `num`, `str`, and `boolean` accessors, so keeping integer columns as `BIGINT` preserved the `int64` values those accessors expect, and `require_review` and `shared` became real `BOOLEAN` columns because the code already wrote Go bools into them and read them through `boolean`.

1. `rebind` in `db.go` converts `?` to `$n` inside `rows` and `exec`, skipping single-quoted literals. The 68 call sites were not edited. The four `tx.Exec` statements that bypass those helpers were converted directly.
2. `s.insert` appends `RETURNING id`, replacing `LastInsertId` at seven call sites. `s.exec` now returns rows affected.
3. `INSERT OR IGNORE` became `ON CONFLICT DO NOTHING`, and `shared=1` became `shared`.
4. `repoLocks sync.Map` became a session-scoped `pg_try_advisory_lock` on a dedicated pooled connection, retaining the previous 30-second wait, 408 on cancellation, and 503 on timeout. The pool was raised to 25 connections because each held lock occupies one for the duration of a Git operation.
5. `shared/schema.sql` became `shared/migrations/001_initial.sql`. Identity columns are `GENERATED BY DEFAULT`, not `GENERATED ALWAYS`, so a restore or a fixture can supply an explicit identifier.
6. `migrate` applies `shared/migrations/*.sql` in name order, each in its own transaction, recorded in `schema_migrations`.

Unique-violation detection moved from matching the string "UNIQUE" to the SQLSTATE `23505` code.

Dependencies changed rather than stayed flat. `go-sqlite3` is replaced by `pgx` v5, still one direct dependency, but it brings five indirect modules where `go-sqlite3` brought none. Against that, `pgx` is pure Go, so `CGO_ENABLED=0` now produces a static binary and the build needs no C compiler.

Backup and restore were rewritten around `pg_dump` and `pg_restore`, preserving the single consistency unit: the helper still holds `exclusive_data` and `quiescent_transfers` across both the dump and the repository copy. `tests/recovery.py` compares row counts and per-table content digests, sorting copied rows so the comparison does not depend on physical row order, which a dump and restore does not preserve.
