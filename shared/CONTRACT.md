...
Start from repository root. Environment: PORT (default 7701), HOST (default 127.0.0.1), DATA_DIR (required configurable, default .data/go), WEB_DIR (default web), SHARED_DIR (default shared), PUBLIC_URL (default http://localhost:PORT). Data: DATA_DIR/gitclub.db from shared/schema.sql; bare Git repositories DATA_DIR/repos/ID.git. All paths must become absolute at startup. Python 3 and Git are runtime dependencies for hook and SSH transport adapters. Package a Linux container.
